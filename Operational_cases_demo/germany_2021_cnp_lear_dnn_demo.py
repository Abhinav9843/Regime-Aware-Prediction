...
The project root is detected automatically from the current repository layout.
"""
...
DATA_ROOT = Path(__file__).resolve().parent / "Sample_Trading_result"
...
def load_txt(path: Path, rows=None, cols=24) -> np.ndarray:
    ...
    if not path.exists():
        raise FileNotoundError(f"Missing file:\n{path}")
...
def load_germany_2021_common_data(data_root: Path):
    ...
    load_2021 = load_txt(
        data_root / "Trading_Result" / "sim_load_data_germany_2021.txt",
        rows=365,
        cols=24,
    )

    solar_2021 = load_txt(
        data_root / "Trading_Result" / "sim_pv_data_germany_2021.txt",
        rows=365,
        cols=24,
    )
